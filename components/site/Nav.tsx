  ...
  return (
    <header className="sticky top-0 z-50 border-b border-plum-line/70 bg-ink/80 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-shell items-center justify-between px-5 sm:px-8">
        <Link href="#top" onClick={close} className="flex shrink-0 items-center gap-2" aria-label="Bokuzu, home">
        ...
        {/* mobile actions: two priority CTAs always visible, plus the hamburger for the rest */}
        <div className="flex items-center gap-1.5 min-[900px]:hidden">
          <Link
            href="/example"
            onClick={close}
            className="shrink-0 whitespace-nowrap rounded-full bg-lime px-2.5 py-2 text-[11px] font-semibold text-ink shadow-glow transition-transform hover:bg-lime-press active:scale-[0.98]"
          >
            Dashboard example
          </Link>
          <Link
            href="/login"
            onClick={close}
            className="shrink-0 whitespace-nowrap rounded-full border border-plum-line px-2.5 py-2 text-[11px] font-medium text-bone transition-colors hover:border-lime hover:bg-lime hover:text-ink"
          >
            Sign in
          </Link>
          <button
            ...
            onClick={() => setOpen((v) => !v)}
            className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-plum-line text-bone transition-colors hover:border-lime"
          >
      ...
      {open && (
        <nav id="mobile-menu" className="border-t border-plum-line/70 bg-ink min-[900px]:hidden">
          <div className="mx-auto flex max-w-shell flex-col px-5 py-3 sm:px-8">
            ...
            {/* Dashboard example + Sign in live on the top bar now; the menu keeps the rest. */}
            <a
              href="#waitlist"
              onClick={close}
              className="mt-3 flex min-h-[44px] items-center justify-center rounded-full border border-plum-line px-4 text-sm font-medium text-bone transition-colors hover:border-lime hover:bg-lime hover:text-ink"
            >
              Request access
            </a>
          </div>
