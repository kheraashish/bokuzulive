...
export async function issueCode(email: string, opts: { sms?: boolean; phone?: string | null } = {}): Promise<{ sent: boolean; devCode?: string }> {
  const code = newCode();
  await saveCode(email, code);
  const body = `Your Bokuzu sign-in code is ${code}. It expires in 10 minutes. If you did not request this, ignore this message.`;
  ...
  return { sent, devCode: sent || process.env.NODE_ENV === "production" ? undefined : code };
}
